...
| I | **Design a ship** | `operator new(0x118)` + default ctor `0x00546EC0` (no args) | **UNSOLVED** — the default ctor gives a tagged but PARTLESS design, very likely unbuildable. `0x005470A0` is NOT a copy ctor, it deserialises a `DSGN` save section and crashed the client when handed a design. Report `[ ]` automate ship design |
