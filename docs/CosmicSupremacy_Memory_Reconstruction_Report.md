...
`CosmicSupremacy_Reconstruction_Report.md`, which now covers the EXE patches, the
server protocol and the save-blob load path.

**Status.** This is no longer the mechanism for multiplayer state sync. The save
blob is: it round-trips byte-exactly, it carries everything the memory-editing
tools write, and the client will load a server-authored blob at startup with no
user interaction. See the "Audit" section of the main report for the evidence and
for the two things the blob does *not* carry.
...
The flat **+50 on space** applies on top of the clicks.

**CORRECTION (September 2026): the three per-unit outputs are stored after all, and the field is
`Planet:96`.** The earlier search for them looked at ints and floats and missed the width they use:
they are **three packed bytes**, one per rate, and a byte-level diff of two homeworlds found them in
one pass. See `Planet:96` in `ejbo_annotations.json`. Every consequence below that rested on
"derived-not-stored" is withdrawn.
...
#### Multiplayer consequence, the gap does not exist (RESOLVED, September 2026)

**This subsection described a real multiplayer gap. It was wrong.** The measurement that settled it
is recorded here rather than the text simply deleted, because the reasoning was sound given a field
nobody had found.

The claim was that food, production and science are recomputed every turn from a single global click
record, so a client would compute every other civ's homeworld output from its own local
customisation and the simulations would drift. All of it rests on those outputs being
derived-not-stored, and they are not: they are three packed bytes at `Planet:96`.

**The controlled experiment.** One galaxy, two homeworlds, both at population 7 with 4 farmers and
food stores at 0, the local civ customised with all 30 increments in food and the rival untouched.
One turn was resolved and both planets read:

| | `Planet:96` byte 0 | farmers | produced | consumed | food after one turn |
|---|---|---|---|---|---|
| local homeworld | 62 | 4 | 248 | 70 | **178** |
| rival homeworld | 32 | 4 | 128 | 70 | **58** |

`delta = farmers x (32 + foodClicks) - population x 10`, exact to the unit on both planets in the
same tick, and the 120-unit gap is exactly `4 x (62 - 32)`. **The rival used the base rate on a
client whose global record held 30 food clicks**, which is the drift scenario failing to happen.

**The click record is not read during a tick.** At turn 1 the record was rewritten from `0,30,0,0`
to `0,10,0,0`, which would give a rate of 42. The next turn resolved at 62 anyway: food went 178 to
356, a gain of 178, against the 98 that live derivation would have produced. So `0x00842AE4` is the
dialog's record of what has been spent, and nothing in the economy consults it.

**And the rates are in the save blob.** `Planet:96` serialises at `PLPR+4`. A capture of that galaxy
holds exactly one `[62,30,40]` triple and one `[32,30,40]` triple, each inside the `PLPR` of the
matching homeworld. A pushed state carries a customised homeworld's economy correctly.

*Civ traits are safe.* `Owner:744…940` is per-`Owner` state, so replicating the `Owner` carries them.

*Homeworld space is safe.* Baked into `Planet:104`, per planet, and in the blob.

*Food, production and science are safe.* `Planet:96`, per planet, and in the blob.

**The cheap prototype fix this section used to recommend, requiring every player to leave
food/production/science at default, is unnecessary and should not be built.**

[x] check whether the per-unit outputs appear in the `Planet:144` citizen records once a planet has
population, **no.** With 7 citizens on each homeworld the records are 16 bytes shaped
`[job, 0, ptr, 0]`, identical in structure between the two civs, with no rate field. The outputs are
per planet, at `Planet:96`, which is what the item was really asking
[x] confirm whether the click record is per-civ or global, **global, and it does not matter.** The
four counts have **no absolute reference anywhere in `.text`**, so they are reached through a
register base as fields of something, and that something sits directly in front of the static
`CustomizeCivilizationDlg` instance, whose vtable `0x00761584` is at `0x00842B00`, 28 bytes past the
record. One static dialog, one record. Since the economy never reads it, per-civ was the wrong
